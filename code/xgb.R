...
if(file.exists(file.path(temp_dir, 'coverage.rds'))){
  ...
  cblb <- lapply(seq_row, function(i){
    ...
    out <- pblapply(seq_len(replications), function(rp){
      ...
      m_train <- as.matrix(dat[, c('Tr', 'X1', 'X2')])
      g_train <- as.matrix(dat[, c('X1', 'X2')])
      # Full sample
      ...
      m1 <- predict(m, newdata = as.matrix(data.frame(Tr = 1, X1 = dat$X1, X2 = dat$X2)))
      m0 <- predict(m, newdata = as.matrix(data.frame(Tr = 0, X1 = dat$X1, X2 = dat$X2)))
      ...
      blb_out <- pblapply(partitions, function(i){
        ...
        cross_dat <- lapply(folds, function(test_idx){
          ...
          m_train <- as.matrix(train_dat[, c('Tr', 'X1', 'X2')])
          g_train <- as.matrix(train_dat[, c('X1', 'X2')])
          g_test <- as.matrix(test_dat[, c('X1', 'X2')])
          
          # Full sample
          m <- xgboost(data = m_train, label = train_dat$y, verbose = 0, nrounds = 10,
                       params = list(objective = 'reg:squarederror'))
          g <- xgboost(data = g_train, label = train_dat$Tr, verbose = 0, nrounds = 10, 
                       params = list(objective = 'binary:logistic'))
          
          prop_score <- predict(g, newdata = g_test)
          m1 <- predict(m, newdata = as.matrix(data.frame(Tr = 1, X1 = test_dat$X1, X2 = test_dat$X2)))
          m0 <- predict(m, newdata = as.matrix(data.frame(Tr = 0, X1 = test_dat$X1, X2 = test_dat$X2)))
