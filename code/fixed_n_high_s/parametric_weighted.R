...
ns <- c(10000)
hyper_grid <- as.data.table(expand.grid(n = c(ns, 50000),
                        gamma = c(0.8),
                        subsets = c(25),
                        prop_form = c('wrong'),
                        out_form = c('correct')))
    ...
    out <- pblapply(seq_len(replications), function(rp){
      ...
      blb_out <- lapply(partitions, function(i){
        ...
        blb_reps <- sapply(seq_len(r), function(bt){
          m_boot <- glm(as.formula(out_formula), data = tmp_dat, family = 'gaussian', weights = M[, r])
          g_boot <- glm(as.formula(prop_formula), data = tmp_dat, family = 'binomial', weights = M[, r])
