  ...
  cblb <- lapply(seq_row, function(i){
    ...
    out <- pblapply(seq_len(replications), function(rp){
      ...
      blb_out <- lapply(partitions, function(i){
        ...
        full_dat <- copy(tmp_dat)

        full_dat$prop_score <- prop_score
        full_dat$m1 <- m1
        full_dat$m0 <- m0
        
        phi1_subset <- (full_dat$Tr/full_dat$prop_score)*(full_dat$y - full_dat$m1) + full_dat$m1
        phi0_subset <- (1 - full_dat$Tr)/(1 - full_dat$prop_score)*(full_dat$y - full_dat$m0) + full_dat$m0
