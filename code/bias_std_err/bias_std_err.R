...
hyper_grid <- as.data.table(expand.grid(n = c(20000, 50000),
                                        gamma = c(0.7),
                                        subsets = c(25),
                                        prop_form = c('correct'),
  ...
  cblb <- lapply(seq_row, function(i){
    ...
    out <- pblapply(seq_len(replications), function(rp){
      ...
      blb_out <- lapply(partitions, function(i){
        ...
        sub_dat <- copy(tmp_dat)

        sub_dat$prop_score <- prop_score
        sub_dat$m1 <- m1
        sub_dat$m0 <- m0
        
        phi1_subset <- (sub_dat$Tr/sub_dat$prop_score)*(sub_dat$y - sub_dat$m1) + sub_dat$m1
        phi0_subset <- (1 - sub_dat$Tr)/(1 - sub_dat$prop_score)*(sub_dat$y - sub_dat$m0) + sub_dat$m0
