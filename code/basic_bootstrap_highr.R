  ...
  cblb <- lapply(seq_row, function(i){
    ...
    out <- pblapply(seq_len(replications), function(rp){
      ...
      blb_out <- lapply(partitions, function(i){
        ...
        # blb_reps <- tau_hat - blb_reps  
        data.table(quantile1 = quantile(blb_reps, 0.025),
                   quantile2 = quantile(blb_reps, 0.975))
      })
      
      blb_out <- rbindlist(blb_out)

      blb_out <- blb_out[, .(quantile1 = mean(quantile1),
                  quantile2 = mean(quantile2))]
      blb_out[, `:=`(tau_hat = tau_hat)]
...
cblb[, `:=`(lower_ci = 2*tau_hat - quantile2, 
            upper_ci = 2*tau_hat - quantile1)][
              , .(coverage = mean(lower_ci <= te & upper_ci  >= te)), by = c('n', 'subsets', 'gamma', 'prop_form', 'out_form')]
